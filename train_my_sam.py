...
def parse_opt():
    ...
    parser.add_argument('-auxiliary_model', type=str, default='BPAT_UNet')
...
def main(opt):
    ...
    if opt.vit_type == "vit_b":
        ...
        checkpoint = f'./save_models/Thyroid_tn3k_fold0/vit_h_1.00_heigh/sam_best.pth'
    else:
        checkpoint = f'./work_dir/SAM/sam_vit_l_0b3195.pth'
    sam = sam_model_registry[opt.vit_type](checkpoint=checkpoint)

    my_sam = sam_model_registry["vit_my_sam"](checkpoint='./save_models/Thyroid_tn3k_fold0/vit_my_sam_1.00/sam_best.pth')
    ...
    if opt.auxiliary_model == 'MySAMModel':
        auxiliary_model = MySAMModel(sam, my_sam, auxiliary_model)

    myModel = MySAMModel(sam, my_sam, auxiliary_model)

    img_mask_encdec_params = list(sam.prompt_encoder.parameters()) + list(
        sam.mask_decoder.parameters()
    )
    optimizer = optim.AdamW(myModel.parameters(), lr=lr, betas=beta, weight_decay=opt.weight_decay)
