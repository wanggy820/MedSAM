...
class MedSAMBox(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        a = (mask_256*255).squeeze().numpy().astype(np.uint8)
        edges_256 = cv2.Canny(a, 100, 200)
        edges_256 = torch.from_numpy(edges_256).unsqueeze(0)



        mask_1024 = self.preprocessMask(mask_np, self.transform_image, self.img_size)
        a = (mask_1024*255).squeeze().numpy().astype(np.uint8)
        edges_1024 = cv2.Canny(a, 100, 200)
        edges_1024 = torch.from_numpy(edges_1024).unsqueeze(0)
        ...
        data = {
            ...
            "edges_256": edges_256/255.0,
            "edges_1024": edges_1024/255.0
        }
        return data
