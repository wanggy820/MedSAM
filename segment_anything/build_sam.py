...
from .modeling.my_image_encoder import MyImageEncoderViT
...
def build_my_sam_vit_b(checkpoint=None):
    return _build_my_sam(
        encoder_embed_dim=1280,
        encoder_depth=32,
        encoder_num_heads=16,
        encoder_global_attn_indexes=[7, 15, 23, 31],
...
def _build_my_sam(
    ...
    image_embedding_size = image_size // vit_patch_size
    sam = Sam(
        image_encoder=MyImageEncoderViT(
            depth=encoder_depth,
            embed_dim=encoder_embed_dim,
            img_size=image_size,
            mlp_ratio=4,
