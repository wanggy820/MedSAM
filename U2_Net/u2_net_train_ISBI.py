...
def main():
    ...
    for img, seg in zip(data["img"], data["seg"]):
        ...
        tra_lbl_name_list.append(datasets_dir + seg)
    ...
    model_file = model_dir + "u2net_bce_best_ISBI.pth"
    ...
    best_loss = 0
    for epoch in range(7, epoch_num):
        net.train()
        total_iou = 0
        running_loss = 0.0
        ...
        print("[epoch: %3d/%3d] train loss: %3f, tar: %3f， IOU:%3f" % (
            epoch + 1, epoch_num, running_loss / len(train_dataloader),
            running_tar_loss / len(train_dataloader), total_iou))
